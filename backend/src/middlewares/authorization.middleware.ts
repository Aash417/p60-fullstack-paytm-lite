...
export const protect = asyncHandler(async (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  if (!token) throw new ApiError(401, 'You are not logged in, Please login to get access');
  ...
  if (!user) next(new ApiError(401, 'The user belonging to this token does not exists.'));
